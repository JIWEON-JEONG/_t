...
class ProjectService:
    ...
    def invite(self, db: Session, user: User, param: InviteProjectRequest) -> int:
        user_project_role: ProjectUserRole = self.project_repository_port.get_role_by_project_and_user(param.project_id, user.id)
        ...
        project: Optional[Project] = self.project_repository_port.get_by_id(param.project_id)
        if project is None:
            raise HTTPException(status_code= 404, detail=f"프로젝트 {param.project_id} 를  찾을 수 없습니다.")

        self.project_repository_port.save_user_role(db, ProjectUserRole.create(param.project_id, param.member_id, param.member_role))
        self.logger.info(f"[프로젝트 초대 성공] 프로젝트 ID: {param.project_id}, 생성자: {user.id}, 멤버: {param.member_id}, 권한: {param.member_role}")
    ...
    def create(self, db: Session, user: User, desc: str) -> int:
        ...
        self.project_repository_port.save_user_role(db, ProjectUserRole.create(project.id, user.id, ProjectRole.PROJECT_OWNER))

        self.logger.info(f"[프로젝트 생성 성공] 프로젝트 ID: {project.id}, 생성자: {user.id}, 회사 ID: {user.company_id}")
        return project
    
    def update(self, db: Session, user: User, param: UpdateProjectRequest) -> int:
        user_project_role: ProjectUserRole = self.project_repository_port.get_role_by_project_and_user(param.project_id, user.id)
        if(not self.check_update_permission(user.role, user_project_role)):
            self.logger.warning(f"[프로젝트 수정 실패] 사용자 {user.id}가 권한 없이 프로젝트({param.project_id}) 수정 시도")
            raise HTTPException(status_code= 403, detail=f"권한이 없습니다.")

        complete: bool = self.project_repository_port.update_project_description(db, param.project_id, param.description)
        if complete == False : 
            raise HTTPException(status_code= 400, detail=f"이미 삭제된 프로젝트입니다.")
    
        self.logger.info(f"[프로젝트 수정 성공] 프로젝트 ID: {param.project_id}, 수정자: {user.id}, 변경된 설명: {param.description}")
        return param.project_id
    
    def delete(self, db: Session, user: User, project_id: int) -> int:
        user_project_role: ProjectUserRole = self.project_repository_port.get_role_by_project_and_user(project_id, user.id)
    ...
    def read(self, db: Session, user: User, project_id: int) -> Project:
        ...
        project: Project = self.project_repository_port.get_by_id(db, project_id)
        return project
