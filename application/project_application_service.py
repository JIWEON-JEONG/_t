...
class ProjectApplicationService:
    ...
    @transactional
    async def create(self, session_: UserSessionDto, param: CreateProjectRequest, db=None) -> int:
    ...
    @transactional
    async def update(self, session_: UserSessionDto, param: UpdateProjectRequest, db=None) -> int:
        user: User = self.user_service.get_user_by_session(db, session_.session_id, session_.ip)
        return self.project_service.update(db, user, param)
    
    @transactional
    async def delete(self, session_: UserSessionDto, param: DeleteProjectRequest, db=None) -> int:
        user: User = self.user_service.get_user_by_session(db, session_.session_id, session_.ip)
        return self.project_service.delete(db, user, param.project_id)
    
    @transactional
    async def invite(self, session_: UserSessionDto, param: InviteProjectRequest, db=None) -> int:
        user: User = self.user_service.get_user_by_session(db, session_.session_id, session_.ip)
        return self.project_service.invite(db, user, param.project_id)
    
    async def read(self, session_: UserSessionDto, project_id: int, db=None) -> int:
        user: User = self.user_service.get_user_by_session(db, session_.session_id, session_.ip)
